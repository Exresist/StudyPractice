...
namespace Ex11
{
    class Program
    {
         ...
         * Решетка, т. е. квадрат из клетчатой бумаги 10x 10 клеток, некоторые клетки в котором вырезаны,
         * совмещается с целым квадратом 10x 10 клеток и через прорези на бумагу наносятся первые буквы текста.
         * Затем решетка поворачивается на 90° и через прорези записываются следующие буквы. Это повторяется еще дважды.
         * Таким образом, на бумагу наносится 100 букв текста.
         * Решетку можно изображать квадратной матрицей порядка 10 из нулей и единиц (нуль изображает прорезь).
         * Доказать, что матрица [a,y]i=i io; /=i ю может служить ключом шифра,
         * если из элементов аи% al0_i+i/, апоЧ+и a10- l+1; 10-/+i в точности один равен нулю.
            ...
            if (CheckMatrixValues(matrix))
            {
                ...
                for (int column = 0; column < order / 2; column++)
                {
                    if (matrix[row][column] + matrix[column][order - 1 - row] // поскольку среди этих только
                                            + matrix[order - 1 - row]
                                                [order - 1 - column] // одно значение 0, а остальные 1, 
                                            + matrix[order - 1 - column][row] != 3) // то сумма должна быть равна 3
                    {
                        return false;
                    }
                }
        ...
        static string
            ReadString(StreamReader inputString, out bool okReading) // чтение строки для шифрования и дешифрования
        {
        ...
        static void Main(string[] args)
        {
            ...
            if (okReading)
            {
                ...
                if (okReading)
                {
                    ...
                    Console.WriteLine("\nЗашифрованная последовательность: " + cryptSymbols);

                    string decryptedSymbols = Decipher(matrix, cryptSymbols);
                    Console.WriteLine("\nРасшифрованная последовательность: " + decryptedSymbols);
                }
            }
